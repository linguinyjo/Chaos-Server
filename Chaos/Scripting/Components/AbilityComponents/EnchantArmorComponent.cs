...
public struct EnchantArmorComponent : IConditionalComponent
{
    ...
    public bool Execute(ActivationContext context, ComponentVars vars)
    {
        ...
        if (item.LevelCircle != options.LevelCircle || 
            item.Template.EquipmentType is not (EquipmentType.Armor or EquipmentType.Gauntlet or EquipmentType.Greaves or EquipmentType.Boots)
        )        
        {
            context.SourceAisling?.SendOrangeBarMessage($"This scroll can only enchant armor of Circle {options.LevelCircle}");
            return false;
        }
